...
const Categories: React.FC<CategoriesProps> = ({ selectedCategory, onSelect }) => {
  ...
  return (
    <section className="flex flex-col gap-4 mt-2 mb-1! mr-1">
      {allCategories.map((cat) => (
        <div
          key={cat.id}
          onClick={() => onSelect(cat.name)}
          className={`px-2 py-2 text-[12px] font-medium rounded-[5px] cursor-pointer flex items-center justify-center min-h-20 w-[30px] ${
